...
def validate_register_name(name: str) -> None:
    ...
    if forbidden_chars:
        raise InvalidRegisterNameError(name, forbidden_chars)


class InvalidRegisterNameError(ValueError):
    def __init__(self, name: str, forbidden_chars: frozenset[str]) -> None:
        chars = ", ".join(f"'{char}'" for char in sorted(forbidden_chars))
        super().__init__(f"Register name '{name}' contains reserved character(s): {chars}")
