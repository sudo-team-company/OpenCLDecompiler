...
def process_arg(offset: int, arg: KernelArgument):
    ...
    if offset % 4 == 0:
        DecompilerData().config_data.offset_to_content[hex(offset)] = RegisterContent(
            value=value, type_=get_reg_type(arg.name), size=arg.basic_size(), data_type=make_asm_type(arg.type_name),expression_node=ExpressionManager().add_kernel_argument(arg, offset)
        )
