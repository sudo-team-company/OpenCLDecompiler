...
from src.register import is_vgpr
...
def make_output_for_linear_region(region, indent):
    ...
    if isinstance(region.start, Node):
        ...
        while True:
            ...
            if len(curr_node.instruction) > 1 and is_vgpr(curr_node.instruction[1]):
                reg = curr_node.instruction[1]
                var = decompiler_data.variables.get(curr_node.state.registers[reg].version)
                if var is not None and var != curr_node.state.registers[reg].val:
