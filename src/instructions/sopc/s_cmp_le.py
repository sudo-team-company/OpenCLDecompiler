...
class SCmpLe(BaseInstruction):
    ...
    def to_print_unresolved(self):
        if self.suffix == 'i32':
            self.decompiler_data.write("scc = (int)" + self.ssrc0 + " <= (int)" + self.ssrc1 + " // s_cmp_le_i32\n")
            return self.node
        if self.suffix == "u32":
            self.decompiler_data.write("scc = " + self.ssrc0 + " <= " + self.ssrc1 + " // s_cmp_le_u32\n")
            return self.node
