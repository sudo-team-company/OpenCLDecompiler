    ...
    def to_fill_node(self):
        if self.suffix == 'b32':
            if self.src1 in self.node.state and \
                    self.node.state[self.src1].type == RegisterType.DIVISION_PT8:
            ...
            if self.src1 in self.node.state and \
                    self.node.state[self.src1].type == RegisterType.DIVISION_PT10:
                new_value = self.node.state[self.src1].val
                return set_reg_value(self.node, new_value, self.vdst, [self.src0, self.src1], self.suffix,
                                     reg_type=RegisterType.UNKNOWN)
            if self.ssrc2 in self.node.state and \
                    self.node.state[self.ssrc2].type == RegisterType.DIVISION_PASS:
            ...
            if self.vdst in self.node.state and \
                    self.node.state[self.vdst] is not None and \
                    self.node.state[self.vdst].type == RegisterType.KERNEL_ARGUMENT_ELEMENT:
                variable = "*" + variable
