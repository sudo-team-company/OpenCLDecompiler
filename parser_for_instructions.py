...
class Decompiler:
    ...
    def process_src(self):
        ...
        for key in list(range(len(offsets) - 1)):
            ...
            self.output_file.write("    __local " + type_of_var + " " + self.lds_vars[offsets[key]][0] + "[" + str(size_var) + "]" + ";\n")
    ...
    def make_output(self, region, indent):
        if region.type == TypeNode.linear:
            ...
            for key in self.variables.keys():
                reg = key[:key.find("_")]
                if region.start.start.parent[0].state.registers[reg] is not None and region.start.start.parent[0].state.registers[reg].version == key and self.variables[key] in self.names_of_vars.keys():
            ...
            for key in self.variables.keys():
                reg = key[:key.find("_")]
                if region.end.start.parent[0].state.registers[reg].version == key and self.variables[key] in self.names_of_vars.keys():
            ...
            for key in self.variables.keys():
                reg = key[:key.find("_")]
                if region.end.start.parent[1].state.registers[reg].version == key and self.variables[key] in self.names_of_vars.keys():
        ...
        if prefix == "ds":
            if root == "add":
                if suffix == "u32":
                    ...
                    if flag_of_status:
                        node.state.registers[name].val = node.state.registers[name].val + " + " + node.state.registers[vdata0].val
                ...
                if suffix == "i64":
                    ...
                    if flag_of_status:
                        if node.state.registers[from_registers].val == "0":
                            node.state.registers[from_registers].val = node.state.registers[name_of_from + str(first_from + 1)].val
                        node.state.registers[to_registers] = \
